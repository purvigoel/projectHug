...
void SpecialBrush::update(Canvas2D *canvas){
    ...
    for (int row = 1; row < canvasHeight - 1; row++){
        for(int col = 1; col<canvasWidth - 1; col++){
    ...
    std::vector<float> scrollNoise;
    scrollNoise.resize(canvasWidth * canvasHeight, 0.0);

    for(int i = 0; i < canvasWidth * (canvasHeight-1); i ++){
        scrollNoise[i] = m_noise[i + canvasWidth];
    }
    int lastRow = canvasWidth *(canvasHeight - 1);
    for(int i = 0; i < canvasWidth; i ++){
        scrollNoise[i + lastRow] = m_noise[i];
    }
    for(int i = 0; i < canvasWidth * canvasHeight; i ++){
        m_noise[i] = scrollNoise[i];
    }
    std::cout << "3 " << std::endl;
    canvas->repaint();
    usleep(10000);
...
std::vector<float> SpecialBrush::noise(int canvasWidth, int canvasHeight){
    ...
    for(int i = 0; i < canvasArea; i++){
       float r = (float)rand()/RAND_MAX * maxRand;
       if(r < dampenRand){
           r = 0.0;
       }
       noiseHolder[i] = r;
    ...
    dampenRand = 5.0;
    // We'll dampen some of the values again, much less aggressively.
    for(int i = 0; i < canvasArea; i++){
       if(noiseHolder[i] < dampenRand){
           noiseHolder[i] = 0.0;
       }
    }
    m_noise = noiseHolder;
