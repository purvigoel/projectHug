...
void Brush::brushDragged(int mouseX, int mouseY, Canvas2D* canvas, bool alphaBlending) {
    ...
    for(int row = startY; row < endY; row ++){
        for(int col = startX; col < endX; col++){
            ...
            // Differentiate our actions based on whether we're doing the alpha fix or not.
            if(alphaBlending){
...
// We blend the saved brush stroke into the canvas before the stroke. We take into
// account the alpha value, which we ignored before.
...
BGRA Brush::blendColor(int maskNum, BGRA canvasColor, bool useAlphaBlending){
    ...
    if(useAlphaBlending){
        ...
        float alpha = getAlpha()/255.0;
        newColor.r = canvasColor.r * (1 - alpha * m_mask[maskNum]) + (alpha * m_mask[maskNum] * getRed());
        newColor.g = canvasColor.g * (1 - alpha * m_mask[maskNum]) + (alpha * m_mask[maskNum] * getGreen());
        newColor.b = canvasColor.b * (1 - alpha * m_mask[maskNum]) + (alpha * m_mask[maskNum] * getBlue());
        newColor.a = 255.0;
    }
    return newColor;
}
