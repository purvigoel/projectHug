...
{
    std::cout << " hi from implicit sphere " << std::endl;
}
ImplicitSphere::~ImplicitSphere()
{
...
glm::vec3 ImplicitSphere::getTextureTarget(glm::vec3 point){
    ...
    if(fabs(point.y + 0.5f) < 0.0001f){
        return glm::normalize(glm::vec3(0.5f,0.0f,0.0f));
    }
    if(fabs(point.y - 0.5f) < 0.0001f){
        return glm::normalize(glm::vec3(0.5f,1.0f,0.0f));
    }
