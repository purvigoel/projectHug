...
// Gather up all the pixel values under the kernel and find the median of them.
// This performs something that looks like a blurring effect.
std::vector<float> FilterMedian::convolve(int kernelR, int kernelC, std::vector<float> in, int width, int height){
    ...
    for (int r = 0; r < height; r++) {
        for (int c = 0; c < width; c++) {
            ...
            // find the median of this vector.
...
// Put the reds, greens, and blues back into BGRA format.
