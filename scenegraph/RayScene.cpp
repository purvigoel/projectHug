...
glm::vec3 RayScene::recolorSceneRecursive(int width, int height, glm::mat4x4 camToWorld, glm::vec4 eye, glm::vec4 d, int recursionDepth, int primToSkip){
    ...
    // Hey TAs, this is where the recursion depth is defined. Please enjoy. Have a nice day!
    if(recursionDepth > 3){
        return color;
    }
    ...
    for(int i = 0; i < numLights; i++){
        if(m_lightInfo[i]->type == LightType::LIGHT_DIRECTIONAL && !settings.useDirectionalLights){
            continue;
        }
        if(m_lightInfo[i]->type == LightType::LIGHT_POINT && !settings.usePointLights){
            continue;
        }

        if(settings.useShadows && !generateHardShadows(resVector, m_lightInfo[i], rayInfo.id)){
            continue;
        }
        ...
        //texture
        if(settings.useTextureMapping && rayInfo.textureImage != nullptr){
        ...
        // impose a threshold. a very, very small threshold.
        if(rvD < 0.00000005f){
            rvD = 0.0f;
        }
    ...
    glm::vec3 recursiveColor = glm::vec3();
    if(settings.useReflection){
        ...
        recursiveColor = glm::vec3(rVal, gVal, bVal);
    }
...
bool RayScene::generateHardShadows(glm::vec4 surfacePoint, CS123SceneLightData *candidate, int primId){
    // no white speckles. no black speckles. NO. SPECKLES.
    ...
    // @shadow special case, we're looking for occlusions closer than the light itself.
    float tToBeat = glm::length(candidate->pos - p);
    ...
    for(int prim = 0; prim < m_primitiveInfo.size(); prim++){
        ...
        // this would check if we are self occluding, but I believe objects CAN be self occluding?
        if(currPrim->id == primId){
            continue;
        }

        float shapeIntersect = findObjectIntersection(*currPrim, p, d);
        if(shapeIntersect < INFINITY && shapeIntersect < tToBeat){
            // welp we were occluded, time to go
            return false;
        }
    }
...
RayInfo RayScene::getCollisionShape(glm::vec4 p, glm::vec4 d, int primToSkip){
    ...
    for(int prim = 0; prim < m_primitiveInfo.size(); prim++){
        ...
        if(shapeIntersect < bestT){
            ...
            rayInfo.res = p + shapeIntersect * d;
